          ...
          <div class="nav-collapse collapse">
            ...
            <div class="btn-group">
  				...
  				<ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
                	<li class="divider"></li>
    				<li><a href="#logout" data-toggle="modal"><i class="icon-off"></i> Salir</a></li>  
...
<div id="logout" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="alert alert-info">Estás seguro que quieres<strong>Salir</strong>?</div>
  </div>
  <div class="modal-footer">
      <button class="btn" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i> Cerrar</button>
      <a href="logout.php" class="btn btn-info"><i class="icon-off"></i> Salir</a>
  </div>
</div>             
        	...
        	<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">                      
                            <thead class=" hero-unit">
                                <tr>
                                    ...
                                    <th><div align="center" style="font-size:16px">Código_Operación</div></th>
                            ...
                            <tbody>
                                    ...
                                    $query = mysql_query("select * from tb_reserva where memberID = '$session_id' and status='reserved'") or die(mysql_error());
                                    while ($row = mysql_fetch_array($query)) {
										$roomID = $row['roomID'];
										$balance =$row['balance'];
										$pre = $row['partial'];
									
                                    $qry = mysql_query("select * from tb_habitaciones where roomID = '$roomID'") or die(mysql_error());
                                    while ($rowroom = mysql_fetch_array($qry)) {
											
										$catid = $rowroom['category_id'];
										$price = $rowroom['price'];
										
                      					 $qr = mysql_query("select * from tb_categoria where category_id = '$catid'") or die(mysql_error());
                              ...
                              <tr>
                               ...
                               <td><?php 
                                ...
                                <a class="btn" href="#resched<?php echo $row['reserveID'];?>" role="button" data-toggle="modal">Volver a programar <i class="icon-edit"></i></a> 
                                <a class="btn btn-info" href="#transfer<?php echo $row['reserveID'];?>" data-toggle="modal">Cambio <i class="icon-home"></i></a>
...
<div id="resched<?php echo $row['reserveID'];?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    ...
    <h3 id="myModalLabel">Cambiar Reservación</h3>
  </div>
  <div class="modal-body"> 
 ...
 <div align="center"> 	
						...
						<span style="margin-right: 11px; color:rgba(255,255,255,1);">Fecha de inicio: <input type="text" class="w8em format-d-m-y highlight-days-67 range-low-today" placeholder="Arrival" name="start" id="sd" value="" maxlength="10" readonly style="width: 210px; margin-left: 15px; border: 1px double #CCCCCC; padding:5px 10px;"/></span><br>
						<span style="margin-right: 11px; color:rgba(255,255,255,1);">Fecha final:<input type="text" class="w8em format-d-m-y highlight-days-67 range-low-today" placeholder="Departure" name="end" id="ed" value="" maxlength="10" readonly style="width: 210px; margin-left: 23px; border: 1px double #CCCCCC; padding:5px 10px;" /></span><br>
<input type="hidden" name="result" id="result" /><br>
  ...
  <div class="modal-footer">
  	<button type="submit" name="resche" onClick="setDifference(this.form);" class="btn btn-info"><i class="icon-check"></i> VERIFICAR DISPONIBILIDAD</button>
    <button class="btn" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i> Cerrar</button>
...
<div id="transfer<?php echo $row['reserveID'];?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div align="center">Número de habitación: <select class="span2" name="roomID">
  <?php 
 	
	$resc = mysql_query("select * from tb_habitaciones where category_id = '$catid' and status = 'available'") or die (mysql_error());
 ...
 <?php }?>
  ...
  <div class="modal-footer">
    ...
    <button type="submit" class="btn btn-info" name="res"><i class="icon-edit"></i> Cambio</button>
    <button class="btn" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i> Cerrar</button>
